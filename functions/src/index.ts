...
const getStripe = (): Stripe => {
    ...
    if (!secretKey) {
        throw new functions.https.HttpsError(
            'failed-precondition',
            'Stripe secret key not configured. Set STRIPE_SECRET_KEY in functions/.env'
        );
    }
...
function getStripePriceId(
    tier: keyof typeof STRIPE_PRICES,
    ...
    if (!priceId) {
        throw new functions.https.HttpsError(
            'failed-precondition',
            `Stripe price ID missing for ${tier} ${billing}. Set it in functions/.env`
        );
    }
    return priceId;
}

function sanitizeReturnUrl(url: string | undefined, fallbackPath: string): string {
...
const CONJURE_PROTECTED_PAIRS: [string, string][] = [
...
function conjureProtect(message: string): string {
...
export const giovannaChat = functions.https.onCall(async (data, context) => {
    // 1. AUTHENTICATE
    if (!context.auth) {
        throw new functions.https.HttpsError(
            'unauthenticated',
...
export const checkApiStatus = functions.https.onCall(async (data, context) => {
...
export const getSharePacket = functions.https.onCall(async (data) => {
...
export const createCheckoutSession = functions.https.onCall(async (data, context) => {
    ...
    const priceId = getStripePriceId(
        tier as keyof typeof STRIPE_PRICES,
