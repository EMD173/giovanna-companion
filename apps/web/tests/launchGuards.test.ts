...
import {
    ...
    shouldResetUsageCycle,
    toDate,
} from '../src/lib/launchGuards.js';
...
test('shouldResetUsageCycle resets when month changes', () => {
    assert.equal(
        shouldResetUsageCycle(new Date('2026-02-28T12:00:00.000Z'), new Date('2026-03-01T12:00:00.000Z')),
        true
    );
});
...
test('shouldResetUsageCycle does not reset within the same month', () => {
    assert.equal(
        shouldResetUsageCycle(new Date('2026-03-01T12:00:00.000Z'), new Date('2026-03-24T12:00:00.000Z')),
        false
    );
});
